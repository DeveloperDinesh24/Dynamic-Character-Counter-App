# ✍️ Dynamic Character Counter

A sleek, state-driven text editor that handles different posting constraints with real-time visual feedback. This project focuses on **State Variables (`useState`)**, **Conditional Logic**, and **Data-Driven UI Styling**.



## 🚀 Live Demo
**Check it out here:** https://dinesh-dynamic-character-counter-app.netlify.app/
...
* **Automated Sync:** Uses `useState` to dynamically update constraints based on the selected post type.
...
* **Lifecycle Control:** Employs `useState` to ensure the UI stays in sync when the user toggles between modes.
